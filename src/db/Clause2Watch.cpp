...
Literal* Clause2Watch::Deduce(Literal* assign) {
    ...
    Literal* compLit = assign->GetComplementLiteral();

    unsigned int newWatchIdx;
    if (compLit == GetWatch1()) {
...
bool Clause2Watch::IsNeedUpdate(Literal* lit) {
    Literal* compLit = lit->GetComplementLiteral();
    if (compLit != GetWatch1() && compLit != GetWatch2()) {
        return false;
    }
